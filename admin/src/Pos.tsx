import PosShifts from './PosShifts';
...
export default function Pos({role}:{role:string}) {
  const [shiftId,setShiftId]=useState<string|null>(null),[shiftBusy,setShiftBusy]=useState(false),[shiftRefresh,setShiftRefresh]=useState(0);
  ...
  const disabled=busy||shiftBusy||pending.current!==null;
  ...
  async function perform(path:string,body:any){
    ...
    try {
      ...
      pending.current=null;setShiftRefresh(v=>v+1);
  ...
  return <div className="pos">
    <nav className="pos-tabs" aria-label="Разделы кассы">{(canSell?['sale','held','receipts','shifts',...(role==='Administrator'?['report']:[])]:['receipts','shifts','report']).map(t=><button disabled={disabled} className={tab===t?'primary':''} key={t} onClick={()=>{setTab(t);setPage(1);setError('');}}>{({sale:'Продажа',held:'Отложенные чеки',receipts:'Чеки',shifts:'Смены',report:'Отчёт'}as any)[t]}</button>)}</nav>
    <PosShifts role={role} history={tab==='shifts'} refresh={shiftRefresh} disabled={busy||pending.current!==null} onCurrent={setShiftId} onBusy={setShiftBusy}/>
